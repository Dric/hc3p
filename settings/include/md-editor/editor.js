...
var titles = {
	...
	'image':					'Insérer une Image (AltGr-I)',
	'quote':					'Citation (Ctrl-\')',
	'blockcode':			'Code (Tab)',
	'ordered-list':		'Liste ordonnée (AltGr-L)',
...
function createIcon(name, options) {
  ...
  var shortcut = options.shortcut || titles[name];
  if (shortcut) {
...
function drawImage(editor) {
  var cm = editor.codemirror;
  var stat = getState(cm);
  $('#sectionEditorLoadImageModal').modal();
  var src = $('#sectionEditorLoadImageModal').attr('data-src');
  var height = $('#sectionEditorLoadImageModal').attr('data-height') || ($(window).height()*0.7);
  var width = $('#sectionEditorLoadImageModal').attr('data-width') || '100%';
  $("#sectionEditorLoadImageModal iframe").attr({
  	'src':src,
  	'height': height,
  	'width': width
  });
  $("#sectionEditorLoadImageModal").on('hidden.bs.modal', function(){
			var val = $('#sectionEditorInsertImageUrl').val();
			if (val){
				var startPoint = cm.getCursor('start');
				cm.setLine(startPoint.line, '![]('+val+')');
				//_replaceSelection(cm, stat.image, '![', ']('+val+')');
			}
		});
